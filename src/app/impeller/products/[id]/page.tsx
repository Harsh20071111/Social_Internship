...
interface ImpellerProductDetailPageProps {
  params: Promise<{ id: string }> | { id: string };
}

export async function generateMetadata({ params }: ImpellerProductDetailPageProps) {
  const resolvedParams = await params;
  const id = resolvedParams.id;
  
  const categoryItem = impellerCategories.find((item) => item.slug === id);
  if (categoryItem) {
    return {
      title: `${categoryItem.title} | Impeller Series | Bhakti Industries`,
...
export default async function ImpellerProductDetailPage({ params }: ImpellerProductDetailPageProps) {
  const resolvedParams = await params;
  const id = resolvedParams.id;
  ...
  return (
    <PuckPage
      ...
      heroImage="/images/impeller-hero.jpg"
      overlayOpacity={0.45}
    ...
    >
      <section className="bg-[#f3f3f3] py-16 md:py-24">
        <div className="mx-auto max-w-[1160px] px-5">
          ...
          >
            <ArrowLeft className="h-4 w-4" /> Back to Impeller Categories
          </Link>
